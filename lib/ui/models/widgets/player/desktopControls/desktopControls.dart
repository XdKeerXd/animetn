...
import 'package:animetn/ui/models/bottomSheets/customControlsSheet.dart';
import 'package:animetn/ui/models/providers/playerDataProvider.dart';
import 'package:animetn/ui/models/providers/playerProvider.dart';
import 'package:animetn/ui/models/snackBar.dart';
import 'package:animetn/ui/models/widgets/ContextMenu.dart';
import 'package:animetn/ui/models/widgets/player/desktopControls/pipMode.dart';
import 'package:animetn/ui/models/widgets/slider.dart';
import 'package:animetn/ui/models/providers/appProvider.dart';
import 'package:animetn/ui/pages/settingPages/subtitle.dart';
...
class _DesktopControlsState extends State<DesktopControls> {
                          ...
                          children: [
                            ...
                            ContextMenu(
                              menuItems: [
                                ...
                                ContextMenuItem(
                                    icon: Icons.snooze,
                                    label: provider.state.sleepTimerMinutes > 0
                                        ? "Sleep: ${provider.state.sleepTimerMinutes}m"
                                        : "Sleep Timer",
                                    onClick: () {
  ...
  ElevatedButton megaSkipButton() {
    ...
    int posInSec = (provider.controller.position ?? 0) ~/ 1000;
    ...
    if (!isAtOp && !isAtEd) return ElevatedButton(onPressed: () {}, child: SizedBox.shrink(), style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, elevation: 0));
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              _sleepTimerOption(context, provider, 0, "Off"),
              _sleepTimerOption(context, provider, 15, "15 Minutes"),
              _sleepTimerOption(context, provider, 30, "30 Minutes"),
              _sleepTimerOption(context, provider, 60, "60 Minutes"),
              _sleepTimerOption(context, provider, 120, "120 Minutes"),
            ],
          ),
        );
      },
  ...
  Widget _sleepTimerOption(BuildContext context, PlayerProvider provider, int minutes, String label) {
    return ListTile(
      title: Text(label, style: TextStyle(color: Colors.white)),
      trailing: provider.state.sleepTimerMinutes == minutes
